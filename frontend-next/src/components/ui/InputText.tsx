 ...
 * Supports 5 states: Default, Active (Focus), Filled, Error, and Disabled.
 * Animation: 300ms ease-out.
 */
export const InputText = React.forwardRef<HTMLInputElement, InputTextProps>(
  ({ label, helperText, error, disabled, showSearchIcon, showCloseIcon, onClear, className, onFocus, onBlur, value, defaultValue, ...props }, ref) => {
    ...
    else if (isFocused) state = 'Active';
    ...
    const styles = {
      Default: {
        ...
        text: 'text-grey950',
        label: 'text-grey950',
        placeholder: 'placeholder:text-grey500',
        icon: 'text-grey500',
      },
      Active: {
        bg: 'bg-grey100',
    ...
    return (
      <div className={`flex flex-col gap-3 w-full ${className || ''}`}>
        ...
        `}>
          ...
          {showCloseIcon && (hasValue || isFocused) && (
            <button 
              type="button"
              onClick={onClear}
